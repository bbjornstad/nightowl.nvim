local kenv = function(n) return require("environment.keys")[n] end
...
return {
    {
        ...
        config = function(_, opts) vim.diagnostic.config(opts) end,
        event = "LspAttach",
        ...
        config = function(_, opts) require("actions-preview").setup(opts) end,
        keys = {
            {
                key_action.preview,
                function()
        ...
        opts = {
            ...
            mappings = {
                ...
                apply_first = {
                    key_action.apply_first, "action=> apply first action"
                },
                quickfix = {
                    key_action.quickfix.quickfix, "action.qf=> quickfix"
                },
                quickfix_next = {key_action.quickfix.next, "action.qf=> next"},
                quickfix_prev = {
                    key_action.quickfix.prev, "action.qf=> previous"
                },
                refactor = {
                    key_action.refactor.refactor, "action.rf=> refactor"
                },
                refactor_inline = {
                    key_action.refactor.inline, "action.rf=> inline"
                },
                refactor_extract = {
                    key_action.refactor.extract, "action.rf=> extract"
                },
                refactor_rewrite = {
                    key_action.refactor.rewrite, "action.rf=> rewrite"
                },
                source = {key_action.source, "action=> source"}
            }
        },
        config = function(_, opts) require("clear-action").setup(opts) end,
        keys = {
            {
        ...
        opts = function(_, opts)
            ...
            -- (default behavior)
            opts.detached = opts.detached or
                                function(winid)
            ...
            opts.indent_lines = mopts({enable = true, icon = "│"},
                                      opts.indent_lines)
            opts.winbar = mopts({
        ...
        config = function(_, opts) require("diaglist").init(opts) end,
        event = "LspAttach",
        keys = {
            {
        ...
        config = function(_, opts) require("corn").setup(opts) end,
        opts = {
            auto_cmds = true,
        ...
        config = function(_, opts) require("error-lens").setup(opts) end,
        keys = {
            {
                key_view.diagnostic.error_lens.toggle,
                function() require("error-lens").toggle() end,
                mode = "n",
