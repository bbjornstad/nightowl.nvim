...
local function winsep_disable_ft()
  ...
  if win_n == 2 then
    ...
    local filetype = vim.api.nvim_buf_get_option(
      vim.api.nvim_win_get_buf(win_id or 0), "filetype")
...
return {
  { "folke/persistence.nvim", enabled = opt.sessions.persistence }, {
  ...
  event = "VeryLazy",
  config = function(_, opts) require("help-vsplit").setup(opts) end,
  opts = {
    always = true,
    side = "right",
    buftype = { "help" },
  ...
  enabled = false,
  keys = {
    { "<leader>ue", false }, { "<leader>uE", false }, {
  ...
  opts = function()
    ...
    local function condition(condition_to, edgy_loc, values)
      ...
      table.insert(opts[edgy_loc], #opts[edgy_loc],
  ...
  config = function(_, opts)
    ...
    local grp = vim.api.nvim_create_augroup("FtDisableFocus",
      { clear = true })
  ...
  opts = {
    autowidth = { enable = false, winwidth = 5, filetype = { help = 2 } },
    ignore = {
  ...
  config = function(_, opts) require("flatten").setup(opts) end
}, {
  "tiagovla/scope.nvim",
  ...
  keys = {
    {
      ...
      function() require("hbac").toggle_pin() end,
      mode = "n",
      desc = "buf.pin=> toggle"
    }, {
    key_buffer.hbac.pin.all,
    function() require("hbac").pin_all() end,
    mode = "n",
    desc = "buf.pin=> toggle"
  }, {
  ...
  config = function(_, opts) require("bufresize").setup(opts) end,
  enabled = true,
  event = "BufWinEnter"
}, {
  ...
  opts = {},
  enabled = true,
  config = function(_, opts) require("accordian").setup(opts) end,
  event = "VeryLazy",
  keys = {
    {
      key_win.accordian,
